...
def _read_image_max_sample(path: Path) -> "int | None":
    """Read pixel-range maximum from image metadata (TIFF tag 281 or PNG tEXt)."""
...
def _load_as_float32_gray(path: Path, fallback_bit_depth: int = 12) -> "np.ndarray | None":
    """Load image as float32 grayscale array normalized to [0, 1]."""
    try:
        with PilImage.open(str(path)) as pil:
...
class _FileScanWorker(QRunnable):
    def __init__(self, sig: _FileScanSignals, images_root: Path, camera: str,
                 start_ns: int, end_ns: int, gen_box: list, my_gen: int):
        super().__init__()
        self._sig = sig
...
class _ImageCanvas(QWidget):
    """Image canvas that allows drawing and selecting ROI rectangles."""

    roi_selected = Signal(int)   # index (-1 = none)
    roi_created  = Signal()
    ...
    def mousePressEvent(self, event):
        ...
        if hit is not None:
            ...
            self._drag_end = QPoint(px, py)
            self._dragging = True
            self.roi_selected.emit(-1)

    def mouseMoveEvent(self, event):
        if self._dragging:
            self._drag_end = QPoint(int(event.position().x()), int(event.position().y()))
            self.update()
    ...
    def mouseReleaseEvent(self, event):
        if not self._dragging or self._drag_start is None:
            return
        self._dragging = False
        end = QPoint(int(event.position().x()), int(event.position().y()))
        ...
        if dx < 5 or dy < 5:
            ...
            self.update()
            return
        x0, y0 = self._d2i(min(self._drag_start.x(), end.x()),
                            min(self._drag_start.y(), end.y()))
        x1, y1 = self._d2i(max(self._drag_start.x(), end.x()),
                            max(self._drag_start.y(), end.y()))
    ...
    def paintEvent(self, event):
        ...
        for i, roi in enumerate(self._rois):
            ...
            c = QColor(roi.color)
            pen = QPen(c, 3 if i == self._selected_idx else 2)
            p.setPen(pen)
            p.setBrush(QBrush(QColor(c.red(), c.green(), c.blue(), 50)))
            p.drawRect(rect)
            font = QFont()
            font.setPointSize(8)
            font.setBold(i == self._selected_idx)
            p.setFont(font)
...
class ROIEditorDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        tb = QHBoxLayout()
        btn_img = QPushButton("Load reference image…")
        btn_img.setStyleSheet(_BTN)
        btn_img.clicked.connect(self._load_image)
        tb.addWidget(btn_img)
        tb.addSpacing(8)
        tb.addWidget(QLabel("Name:"))
        self._name_edit = QLineEdit()
        self._name_edit.setFixedWidth(130)
        ...
        tb.addStretch(1)
        btn_load_j = QPushButton("Load JSON…")
        btn_save_j = QPushButton("Save JSON…")
    ...
    def _hint_text(self) -> str:
        return (f"Drag to draw ROIs. Click to select.  "
                f"Total: {len(self._rois)} ROIs defined.")
    ...
    def _load_image(self):
        ...
        if not path:
            return
        try:
            arr = _load_as_float32_gray(Path(path))
            if arr is None:
    ...
    def _save_json(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save ROIs", "rois.json", "JSON (*.json);;All files (*)")
        if not path:
            return
        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump({"version": 1, "rois": [r.to_dict() for r in self._rois]}, f, indent=2)
        except Exception as exc:
...
class _TimelineTab(QWidget):
    sample_selected = Signal(int)  # index into sample_points
    ...
    def update_data(self, sample_points: list, rois: list, threshold: float):
        ...
        self._ax.set_title("Pulser alive (green) / dead (red) timeline  —  click to inspect")
...
class _PreviewLoadSig(QObject):
    ready = Signal(object, int)  # (QPixmap|None, gen)
...
class PulserMonitorWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        # Source
        lv.addWidget(_group_label("Images source"))
        src_row = QHBoxLayout()
        self._root_combo = _NoScrollComboBox()
        for name in IMAGES_ROOT_OPTIONS:
            self._root_combo.addItem(name)
        self._root_combo.currentTextChanged.connect(self._on_root_changed)
        btn_browse = QPushButton("Browse…")
        btn_browse.setFixedWidth(65)
        btn_browse.clicked.connect(self._browse_root)
        src_row.addWidget(self._root_combo, 1)
        src_row.addWidget(btn_browse)
        lv.addLayout(src_row)
        self._root_lbl = QLabel(str(self._images_root))
        self._root_lbl.setStyleSheet("font-size:9px;color:#666;")
        self._root_lbl.setWordWrap(True)
        lv.addWidget(self._root_lbl)

        # Camera
        lv.addWidget(_hsep())
        lv.addWidget(_group_label("Camera folder"))
        cam_row = QHBoxLayout()
        self._cam_filter = QLineEdit()
        self._cam_filter.setPlaceholderText("Filter cameras…")
        self._cam_filter.textChanged.connect(self._filter_cams)
        btn_reload = QPushButton("Reload")
        btn_reload.setFixedWidth(58)
        btn_reload.clicked.connect(self._reload_cams)
        cam_row.addWidget(self._cam_filter, 1)
        cam_row.addWidget(btn_reload)
        lv.addLayout(cam_row)
        self._cam_list = QListWidget()
        self._cam_list.setMaximumHeight(110)
        self._cam_list.setAlternatingRowColors(True)
        lv.addWidget(self._cam_list)
        ...
        # ROIs
        lv.addWidget(_hsep())
        lv.addWidget(_group_label("ROIs"))
        roi_row = QHBoxLayout()
        btn_edit = QPushButton("Edit ROIs…")
        btn_edit.setStyleSheet(_BTN)
        btn_edit.clicked.connect(self._open_roi_editor)
        btn_load_j = QPushButton("Load JSON…")
        btn_load_j.clicked.connect(self._load_rois_json)
        roi_row.addWidget(btn_edit)
        roi_row.addWidget(btn_load_j)
        lv.addLayout(roi_row)
        self._roi_lbl = QLabel("No ROIs defined")
        self._roi_lbl.setStyleSheet("font-size:10px;color:#555;")
        lv.addWidget(self._roi_lbl)
        ...
        for label, val in [("Median of ROIs", "median"),
        ...
        # Sampling
        lv.addWidget(_hsep())
        lv.addWidget(_group_label("Sampling"))
        self._samp_group = QButtonGroup(self)
        rb_n = QRadioButton("Every Nth image:")
        rb_n.setChecked(True)
        rb_n.setProperty("samp_mode", "every_n")
        self._rb_n = rb_n
        rb_int = QRadioButton("One per N minutes:")
        rb_int.setProperty("samp_mode", "interval")
        self._samp_group.addButton(rb_n)
        self._samp_group.addButton(rb_int)
        n_row = QHBoxLayout()
        n_row.addWidget(rb_n)
        self._samp_n_sb = QSpinBox()
        self._samp_n_sb.setRange(1, 10000)
        self._samp_n_sb.setValue(1)
        self._samp_n_sb.setFixedWidth(65)
        n_row.addWidget(self._samp_n_sb)
        lv.addLayout(n_row)
        int_row = QHBoxLayout()
        int_row.addWidget(rb_int)
        self._int_sb = QSpinBox()
        self._int_sb.setRange(1, 1440)
        self._int_sb.setValue(10)
        self._int_sb.setFixedWidth(65)
        int_row.addWidget(self._int_sb)
        lv.addLayout(int_row)
    ...
    def _update_roi_lbl(self):
        n = len(self._rois)
        self._roi_lbl.setText(f"{n} ROI{'s' if n != 1 else ''} defined")
    ...
    def _selected_camera(self) -> "str | None":
        items = self._cam_list.selectedItems()
        if items:
            return items[0].text()
        if self._cam_list.count() == 1:
            return self._cam_list.item(0).text()
        return None

    # ── SLOTS ────────────────────────────────────────────────────────────────

    def _on_root_changed(self, name: str):
        self._images_root = IMAGES_ROOT_OPTIONS.get(name, IMAGES_ROOT_OPTIONS["Lab"])
        self._root_lbl.setText(str(self._images_root))
        self._all_cameras.clear()
        self._cam_list.clear()

    def _browse_root(self):
        path = QFileDialog.getExistingDirectory(
            self, "Select images root", str(self._images_root))
        if path:
            self._images_root = Path(path)
            self._root_lbl.setText(str(self._images_root))
            self._root_combo.setCurrentIndex(-1)
            self._all_cameras.clear()
            self._cam_list.clear()

    def _reload_cams(self):
        start_utc = self._start_dt.astimezone(timezone.utc)
        end_utc = self._end_dt.astimezone(timezone.utc)
        cur = start_utc.replace(minute=0, second=0, microsecond=0)
        end_h = end_utc.replace(minute=0, second=0, microsecond=0)
        cams = set()
        checked = 0
        while cur <= end_h and checked < 96:
            folder = (self._images_root / str(cur.year) / str(cur.month)
                      / str(cur.day) / str(cur.hour))
            try:
                if folder.exists():
                    with os.scandir(str(folder)) as it:
                        for e in it:
            ...
            cur += timedelta(hours=1)
            checked += 1
        self._all_cameras = sorted(cams)
        self._filter_cams(self._cam_filter.text())
        self._log_msg(f"Found {len(cams)} camera folder(s)")

    def _filter_cams(self, text: str):
        self._cam_list.clear()
        filt = text.lower().strip()
        for cam in self._all_cameras:
            if not filt or filt in cam.lower():
                self._cam_list.addItem(cam)
    ...
    def _open_roi_editor(self):
        dlg = ROIEditorDialog(self._rois, self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            self._rois = dlg.result_rois()
    ...
    def _load_rois_json(self):
        ...
        try:
            with open(path, encoding="utf-8") as f:
                data = json.load(f)
            self._rois = [RoiDefinition.from_dict(r) for r in data.get("rois", [])]
            self._update_roi_lbl()
            self._log_msg(f"Loaded {len(self._rois)} ROIs from {Path(path).name}")
        except Exception as exc:
            QMessageBox.warning(self, "Load ROIs", f"Failed:\n{exc}")
    ...
    def _start_scan(self):
        ...
        if not camera:
            QMessageBox.warning(self, "Scan",
                                ...
                                "Click 'Reload' to discover cameras for the current time range.")
            return
        if not self._rois:
            QMessageBox.warning(self, "Scan",
                                "No ROIs defined. Click 'Edit ROIs…' to draw them on a reference image.")
            return
        self._results.clear()
        self._gen_box[0] += 1
        ...
        self._status_lbl.setText("Enumerating files…")
        self._log_msg(f"Scan started: camera={camera}, "
        ...
        worker = _FileScanWorker(self._file_sig, self._images_root, camera,
                                 start_ns, end_ns, self._gen_box, my_gen)
        QThreadPool.globalInstance().start(worker)

    def _apply_sampling(self, files: list) -> list:
        if not files:
            return files
        if self._rb_n.isChecked():
            n = self._samp_n_sb.value()
            return files[::n]
        else:
            interval_ns = self._int_sb.value() * 60 * 1_000_000_000
            result = []
            last_ts = -interval_ns
            for ts, path in files:
                if ts - last_ts >= interval_ns:
    ...
    def _on_files_ready(self, files: list, my_gen: int):
        ...
        if not files:
            self._status_lbl.setText("No images found in the selected range.")
            self._btn_scan.setEnabled(True)
            self._btn_stop.setEnabled(False)
            self._prog.setVisible(False)
            self._log_msg("No images found.")
            return
        self._prog.setRange(0, len(files))
        self._status_lbl.setText(f"Scanning 0/{len(files)}…")
        self._log_msg(f"Enumerated {len(files)} files to scan")
        ...
        worker = _ScanWorker(
            self._scan_sig, files, list(self._rois),
    ...
    def _on_progress(self, done: int, total: int):
        ...
        self._status_lbl.setText(f"Scanning {done}/{total}…")
    ...
    def _on_finished(self):
        ...
        self._prog.setVisible(False)
        n = len(self._results)
        self._status_lbl.setText(f"Done — {n} samples")
        self._log_msg(f"Scan complete: {n} sample points")
        if self._results:
    ...
    def _export_csv(self):
        if not self._results:
            return
        path, _ = QFileDialog.getSaveFileName(
            self, "Export CSV", "pulser_scan.csv", "CSV (*.csv);;All files (*)")
        if not path:
            return
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                header = ["timestamp_ns", "datetime_prague"]
                for roi in self._rois:
                    header += [f"{roi.name}_brightness", f"{roi.name}_norm", f"{roi.name}_alive"]
                w.writerow(header)
                for sp in self._results:
                    dt = _ns_to_dt(sp.ts_ns)
                    row = [sp.ts_ns, dt.strftime("%Y-%m-%d %H:%M:%S")]
